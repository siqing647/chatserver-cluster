...
void doLoginResponse(json& responsejs) {
    if (0 != responsejs["errno"].get<int>()) { // 登录失败
        ...
        // 记录当前用户的好友列表信息
        if (responsejs.contains("friends")) { // 判断是否包含 friends 字段
            g_currentUserFriendList.clear();    // 清空上次登录的好友列表
        ...
        // 记录当前用户的群组列表信息
        if (responsejs.contains("groups")) { // 判断是否包含 groups 字段
            g_currentUserGroupList.clear();    // 清空上次登录的群组列表
